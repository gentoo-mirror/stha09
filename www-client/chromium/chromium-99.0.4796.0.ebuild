...
src_prepare() {
	...
	local PATCHES=(
		...
		"${FILESDIR}/chromium-99-AXApiType-include.patch"
...
src_configure() {
	...
	if [[ ${CHROMIUM_FORCE_CLANG} == yes ]] && ! tc-is-clang || use libcxx; then
		...
		CC=${CBUILD:-${CHOST}}-clang
		CXX=${CBUILD:-${CHOST}}-clang++
	...
	if tc-is-clang; then
		...
		if tc-is-cross-compiler; then
			export BUILD_CXXFLAGS="${BUILD_CXXFLAGS} -Wno-unknown-warning-option"
			export BUILD_CFLAGS="${BUILD_CFLAGS} -Wno-unknown-warning-option"
		fi
	fi
