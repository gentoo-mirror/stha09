...
COMMON_SNAPSHOT_DEPEND="
	...
	dev-libs/libxslt:=
	test? ( >=media-libs/fontconfig-2.14:= )
...
src_prepare() {
	...
	if use test; then
		ln -s "${WORKDIR}/test_fonts" "third_party/test_fonts/test_fonts" || die
		rm -r third_party/hunspell_dictionaries || die
		ln -s "${WORKDIR}/hunspell_dictionaries" "third_party/hunspell_dictionaries" || die
		for file in generate_fontconfig_caches.cc BUILD.gn; do
			sed -i -e "s|cache-7|cache-8|g" "third_party/test_fonts/fontconfig/${file}" || die
		done
	fi
	...
	local PATCHES=(
		...
		"${FILESDIR}/chromium-108-enable-c++20.patch"
		"${FILESDIR}/chromium-108-dawn-bitset.patch"
		"${FILESDIR}/chromium-108-to_address-namepsace.patch"
		"${FILESDIR}/chromium-108-AutomationInternalCustomBindings-alias-r1.patch"
		"${FILESDIR}/chromium-108-v8-pair.patch"
		"${FILESDIR}/chromium-108-blink-template.patch"
		"${FILESDIR}/chromium-108-crashpad-template.patch"
		"${FILESDIR}/chromium-108-view_factory-template.patch"
		"${FILESDIR}/chromium-108-FirstPartySetsHandler-template.patch"
		"${FILESDIR}/chromium-108-RenderFrameObserverTracker-template.patch"
		"${FILESDIR}/chromium-108-SystemMonitor-template.patch"
