...
IUSE="debug doc exegesis gold libedit +libffi build-man ncurses test xar xml z3
	kernel_Darwin ${ALL_LLVM_TARGETS[*]}"
REQUIRED_USE="|| ( ${ALL_LLVM_TARGETS[*]} )"
RESTRICT="!test? ( test )"
SRC_URI+=" !build-man? ( https://dev.gentoo.org/~sultan/distfiles/sys-devel/llvm/llvm-${PV}-manpages.tar.xz )"
...
BDEPEND="
	...
	$(python_gen_any_dep '
		build-man? ( dev-python/sphinx[${PYTHON_USEDEP}] )
...
src_unpack() {
	llvm.org_src_unpack
	if ! use build-man; then
		unpack "llvm-${PV}-manpages.tar.xz"
	fi
}
