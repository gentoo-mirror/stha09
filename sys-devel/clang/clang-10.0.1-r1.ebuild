...
inherit cmake llvm llvm.org multilib-minimal pax-utils unpacker \
...
IUSE="build-man debug default-compiler-rt default-libcxx default-lld doc
	+static-analyzer test xml kernel_FreeBSD ${ALL_LLVM_TARGETS[*]}"
REQUIRED_USE="${PYTHON_REQUIRED_USE}
	|| ( ${ALL_LLVM_TARGETS[*]} )"
RESTRICT="!test? ( test )"
SRC_URI+=" !build-man? ( https://dev.gentoo.org/~sultan/distfiles/sys-devel/clang/clang-${PV}-manpages.tar.xz )"
...
src_unpack() {
	llvm.org_src_unpack
	if ! use build-man; then
		unpack "clang-${PV}-manpages.tar.xz"
	fi
}
...
multilib_src_configure() {
	...
	if multilib_is_native_abi; then
		mycmakeargs+=(
			...
			-DLLVM_ENABLE_SPHINX=$(usex build-man)
		)
		use build-man && mycmakeargs+=(
			-DLLVM_BUILD_DOCS=ON
...
multilib_src_install_all() {
	...
	if ! use build-man; then
		insinto "/usr/lib/llvm/${SLOT}/share/man"
		doins -r "${WORKDIR}/man/man1"
	fi
